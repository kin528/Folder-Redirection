...
// Global variable to track folder history
let folderHistory = [];

// Function to load folders
async function loadFolders(parentID = null, isDeleted = false) {
    try {
        // If navigating into a folder, add the current parentID to history
        if (parentID && folderHistory[folderHistory.length - 1] !== parentID) {
            folderHistory.push(parentID);
        }
...
// Display folders with a Back button
function displayFolders(folders, parentID) {
    folderList.innerHTML = "";

    // Add a "Back" button if not at the root folder
    if (parentID) {
        ...
        backButton.addEventListener("click", () => {
            ...
            const previousFolderID = folderHistory[folderHistory.length - 1] || null; // Get the previous folder ID or null for root
            loadFolders(previousFolderID);
        });
    ...
    folders.forEach((folder) => {
        ...
        folderIcon.innerHTML = "&#128193;"; // Unicode for folder icon
